...
Citizen.CreateThread(function()
	Wait(1000)
	TriggerServerEvent("han_report:GetReports")
	if Config.ESX then
		while ESX == nil do
...
RegisterNUICallback("action", function(data)
	...
	if action == "createNewReport" then
		...
		if Config.TakeScreenshot then
			if Config.TakeScreenshot ~= "" and Config.TakeScreenshot ~= "https://discord.com/api/webhooks/869986128697098250/gSMn3qBcvuL14OFW3-kSGocfHd5eRAPTlglE4ax3Sdam47FeMY1AbvtguPsofH6B-R28" then
				url = false
		...
		TriggerServerEvent("han_report:NewReport",list)
	elseif action == "gotoplayer" then
		if ReportList[data.id] then
			if onesync then
				TriggerServerEvent("han_report:TeleportReport",data.id,true)
			else
				...
				if active then
					...
					TriggerServerEvent("han_report:TeleportReport",id)
				else
					if Config.ESX then
	...
	elseif action == "solvedreport" then
		...
		if reportinfo then
			if reportinfo.solved ~= true or can then
				TriggerServerEvent("han_report:ReportSolved",id,can)
			end
		end
	end
end)
...
AddEventHandler('han_report:ReportSolved', function(id,more)
...
AddEventHandler('han_report:GetReports', function(reports,osync,staff)
	admin = staff
	ReportList = reports
	onesync = osync
end)

RegisterNetEvent('han_report:AddToList')
AddEventHandler('han_report:AddToList', function(newreport)
	if admin then
		SendNUIMessage({
			action = "notification",
		})
